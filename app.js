(() => {
  ...
  function render(preserveScroll = false) {
    ...
    if (screen === 'welcome') {
      ...
      app.innerHTML = `<section class="card"><p class="kicker">填写完成</p><h1>回看一遍，再带走它</h1><p class="lead">你的回答仍只保存在当前设备。如需调整，也可以随时返回修改；确认无误后，可导出Markdown文件并发送给识己，我们会据此为你制作完整报告。</p><div class="review-list">${STEPS.map(([number, label, title], index) => `<button data-edit="${index}"><span>${number}</span><div><strong>${label}</strong><small>${title}</small></div><b>修改</b></button>`).join('')}</div><div class="export-panel"><p>导出前，请确认没有身份证号、详细住址、账户信息、完整医疗记录或其他不必要的敏感资料。</p><button class="primary export-button" data-action="export">保存为本地 Markdown</button>${notice ? `<span class="export-notice" role="status">${notice}</span>` : ''}</div><div class="review-actions"><button class="text-button" data-action="last">← 返回最后一题</button><button class="danger-link" data-action="clear">清空全部内容</button></div></section>`;
    }
  ...
  function exportMarkdown() {
    ...
    const filename = `识己-经历与状态信息-${date}.md`;
